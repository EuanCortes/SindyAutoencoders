...
def train_network(training_data, val_data, params):
    ...
    """
    # Preload saved model weights if specified
    if params.get("preload") is not None:
        if os.path.exists(params["preload"]):
            print(f"Loading pre-trained weights from {params['preload']}")
            net.load_state_dict(torch.load(params["preload"], map_location=device))
        else:
            print(f"Preload file {params['preload']} not found. Starting with random initialization.")
    """
    # Define optimizer
    ...
    for epoch in range(params['max_epochs']):
        ...
        if params['sequential_thresholding'] and (epoch % params['threshold_frequency'] == 0) and (epoch > 0):
            with torch.no_grad():
                ...
                # Save model parameters during thresholding
                """save_path = params.get("save_intermediate", "intermediate_model.pt")
